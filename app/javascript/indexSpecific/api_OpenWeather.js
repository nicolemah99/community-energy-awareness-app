(function () {
    ...
    function displayWeather(data) {
        ...
        locationElement.textContent = `${location}, AK`; // Consider getting the state from the data if available
        tempElement.textContent = `${Math.round(temp)} °F`;
        feelsLikeElement.textContent = `Feels like ${Math.round(feels_like)} °F`;
    ...
    fetch(`${BASE_URL}lat=${lat}&lon=${lon}&units=${UNITS}&appid=${API_KEY}`)
        .then(response => {
            if (!response.ok) {
